 ...
 * 
  datas = [{
          ...
          stationImageRotationDeg: 0,  //图片的旋转角度
          stationImageWidth: 14,  //图片的宽
          stationImageHeight: 14,  //图片的高
        }]
 */
L.RH.ZdzLayer = L.RH.ZrBaseLayer.extend({
    ...
    options: {
        ...
        windVaneStyle: { //方向标样式（遵循canvas的样式定义）
            fill: 'black',
            stroke: 'black',
            strokeWidth: 1
        },
        ...
        mouseoverHandler: function (element) {},
    ...
    drawOne: function (rootGroup, element, fromPixel) {
        ...
        const datasColor = element.datasColor || this.options.datasColor;
        ...
        if (this.options.showWind && element.windSD) {
            var wdx = this.options.windLeftOffset,
                wdy = this.options.windTopOffset;
            rootGroup.add(new zrender.WindVane({
                style: this.options.windVaneStyle,
                shape: {
                    speed: element.windSD.speed
                },
                position: [fromPixel.x + wdx, fromPixel.y + wdy],
        ...
        if (element.stationImageUrl) {
            ...
            var rotationDeg = typeof element.stationImageRotationDeg == 'number' ? element.stationImageRotationDeg : 0;
            const px = fromPixel.x - imgW / 2;
            const py = fromPixel.y - imgH / 2;
            var imgObj = new zrender.Image({
                style: {
                    x: px,
                    y: py,
                ...
                rotation: -Math.PI / 180 * rotationDeg,
                origin: [fromPixel.x, fromPixel.y],
                onclick: (function () {
                    this.options.clickHandler && this.options.clickHandler(element)
                }).bind(this)
            });
            imgObj.on('mouseover', function () {
                typeof self.options.mouseoverHandler == 'function' && self.options.mouseoverHandler(element)
            }).on('mouseout', function () {
                typeof self.options.mouseoutHandler == 'function' && self.options.mouseoutHandler(element)
            });

            rootGroup.add(imgObj);
        } else {
